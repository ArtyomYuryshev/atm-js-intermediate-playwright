...
test.describe('Cloud Calculator. 4 types Smoke', () => {
    ...
    test('@mobile | Should be able to add Compute Engine, Cloud Storage, BigQuery and Kubernetes Engine to calculator', async ({
        calculatorPage,
    }) => {
        await waitAndClick(calculatorPage.addEstimatePopup.mobileAddEstimateButton);
        await calculatorPage.addEstimatePopup.mobileAddEstimationModalWindow.waitFor();
    
        await waitAndClick(calculatorPage.addEstimatePopup.computeEngineElement);
        await calculatorPage.costDetails.instanceCard.waitFor();
    
        await calculatorPage.addEstimatePopup.mobileAddEstimateButton.click();
        await calculatorPage.addEstimatePopup.mobileAddEstimationModalWindow.waitFor();
