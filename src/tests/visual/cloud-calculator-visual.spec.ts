...
test.describe('Cloud Calculator Visual Tests', () => {
    ...
    test('Visual test for "Add to this estimate" pop-up', async ({ calculatorPage }) => {
        ...
        await expect(calculatorPage.addEstimatePopup.addEstimationModalWindow).toHaveScreenshot('add-estimate-popup.png', { maxDiffPixelRatio: 0.03 });
    });
    ...
    test('Visual test for cost in header', async ({ calculatorPage }) => {
        ...
        await expect(calculatorPage.configurationComponent.costInHeader).toHaveScreenshot('cost-in-header.png', { maxDiffPixelRatio: 0.03 });
    });
});